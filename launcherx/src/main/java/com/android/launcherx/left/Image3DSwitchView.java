...
public class Image3DSwitchView extends ViewGroup {
    ...
    private static final int CODE_IMAGE = 001;
...
//                    scrollToNext();
    ...
    private void timer() {
        /**
         * schedule:计划表的意思，此处表示未timer，安排工作，表示定制后执行的任务
         *
         * 第一个参数：TimerTask：要执行的任务
         *
         * 第二个参数：long delay:首次执行该任务的事件
         *
         * 第三个参数：long period:执行的周期
         */
        mTimer.schedule(new TimerTask() {
            ...
            public void run() {
                // 要 执行的任务：给主线程发送空消息
                mHandler.sendEmptyMessage(CODE_IMAGE);
            }
        }, 0, 60000);
        // 立刻向主线程发送空消息，之后每隔2秒再次向主线程发送消息。
    }
        ...
        if (mScroller.isFinished()) {
            ...
            switch (action) {
                case MotionEvent.ACTION_DOWN:
                    mHandler.removeMessages(CODE_IMAGE);
                    // 记录按下时的横坐标
                    mLastMotionX = x;
                    break;
